...
[![minzipped size](https://img.shields.io/bundlephobia/minzip/logscope)](https://bundlephobia.com/package/logscope)
[![types included](https://img.shields.io/npm/types/logscope.svg)](https://www.npmjs.com/package/logscope)
[![zero dependencies](https://img.shields.io/badge/dependencies-0-brightgreen.svg)](https://www.npmjs.com/package/logscope?activeTab=dependencies)
...
app.use(logscope.express({ idHeader: "x-request-id" })); // one scope per request
...
The context lives in a `LogScope`. The exported `logscope` is a shared instance; use `new LogScope()` for an isolated one. The context is injected into your logs **at log time**, through each logger's own hook, so a value you add in the middle of a request still shows up in later lines.
...
const logger = winston.createLogger({
  format: winston.format.combine(logscope.winston(), winston.format.json()),
  transports: [new winston.transports.Console()],
});

app.use(logscope.express({ idHeader: "x-request-id" }));
app.use((req, res, next) => { logscope.put("userId", req.user?.id); next(); });
...
const logger = pino({ mixin: logscope.pino() });

app.use(logscope.express());
...
// fastify
fastify.addHook("onRequest", (req, reply, done) => logscope.run({}, () => { logscope.ensureId(); done(); }));

// or wrap any unit of work: a queue job, a cron tick, a script
await logscope.run({ jobId }, async () => {
  logscope.put("attempt", 1);
  await handle();
});
```
...
```ts
export const log = logscope.bind(baseLogger);
log.info("carries context"); // a child with the live context is used per call
```

---

## Why logscope (and where it does not fit)
...
| **MDC (`put`/`get`) auto-injected into winston _or_ pino, framework-agnostic, at log time, zero-dep** | **logscope** |

logscope's wedge is the last row. It is not a logger and it is not an HTTP logger: it is the **context layer** that feeds the logger you already have. It stays out of the way (no wrapper you thread around, no client to adopt), injects at log time (mid-request `put`s show up), and is not tied to winston, to pino, or to express.
...
```ts
const scope = new LogScope({ idKey?, idFactory?, defaults? });
...
logscope is the successor to two of my 2021 experiments in contextual logging, and it exists because each taught the next one something:
...
logscope merges the good ideas from both onto a modern, tested, zero-dependency core, and injects context at log time. **`express-session-logger` is deprecated in favor of logscope.** The full story, problems and fixes and all, is written up as a series (linked from the package page).
